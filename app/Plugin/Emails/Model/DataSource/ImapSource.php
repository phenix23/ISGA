...
class ImapSource extends DataSource {
    ...
    protected $_connectionString = '{imap.gmail.com:993/imap/ssl}INBOX';
    ...
    public function connect($Model, $query) {
        ...
        switch ($this->config['type']) {
            case 'imap':
                $this->_connectionString = sprintf(
                        '{%s:%s%s%s}', $this->config['server'], $this->config['port'], @$this->config['ssl'] ? '/ssl' : '', @$this->config['connect'] ? '/' . @$this->config['connect'] : '', @$this->config['mailbox'] ? @$this->config['mailbox'] : ''
                );
                break;
        ...
        try {
            ...
            while (($retries++) < $this->config['retry'] && !$this->thread) {
                  //debug($_SESSION);die();
                /*
                 * configuration de compte gmail pour protocole IMAP
                 * IMAP server: imap.gmail.com
                  ...
                  User name: your Gmail account name
                  Password: your Gmail password
                  * "test.messagrie", "Eclipse1091"
                 * 
                 */
                //debug(str_replace("@gmail.com",'', $_SESSION['UserAuth']['User']['email']));die();
                $this->Stream = @imap_open("{imap.gmail.com:993/ssl/imap}INBOX", str_replace("@gmail.com",'', $_SESSION['UserAuth']['User']['email']), "Eclipse1091");
