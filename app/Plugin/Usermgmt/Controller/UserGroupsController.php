	...
	public function addGroup() {
		if ($this->request -> isPost()) {
			...
			if ($this->UserGroup->addValidate()) {
				...
				$this->Session->setFlash(__('The group is successfully added'),'message');
	...
	public function editGroup($groupId=null) {
		if (!empty($groupId)) {
			if ($this->request -> isPut()) {
				...
				if ($this->UserGroup->addValidate()) {
					$this->UserGroup->save($this->request->data,false);
					$this->Session->setFlash(__('The group is successfully updated'),'message');
	...
	public function deleteGroup($groupId = null) {
		if (!empty($groupId)) {
			if ($this->request -> isPost()) {
				...
				if($users) {
					$this->Session->setFlash(__('Sorry some users are associated with this group, You cannot delete'),'message');
					$this->redirect('/allGroups');
				}
				if ($this->UserGroup->delete($groupId, false)) {
					$this->Session->setFlash(__('Group is successfully deleted'),'message');
				}
			}
			$this->redirect('/allGroups');
		} else {
