...
class UsersController extends UserMgmtAppController {
		...
		if ($this->request -> isPost()) {
			...
			if($this->User->LoginValidate()) {
				...
				if (empty($user)) {
					...
					if (empty($user)) {
						$this->Session->setFlash(__('Incorrect Email/Username or Password'),'message');
						return;
					}
				}
				// check for inactive account
				if ($user['User']['id'] != 1 and $user['User']['active']==0) {
					$this->Session->setFlash(__('Sorry your account is not active, please contact to Administrator'),'message');
					return;
				}
				// check for verified account
				if ($user['User']['id'] != 1 and $user['User']['email_verified']==0) {
					$this->Session->setFlash(__('Your registration has not been confirmed please verify your email or contact to Administrator'),'message');
					return;
				}
	...
	public function logout() {
		...
		$this->Session->setFlash(__('You are successfully signed out'),'message');
			...
			if ($this->request -> isPost()) {
				...
				if ($this->User->RegisterValidate()) {
					if (!isset($this->data['User']['user_group_id'])) {
						...
						$this->Session->setFlash(__('Please select correct register as'),'message');
						return;
					}
					...
					if (isset($this->request->data['User']['email_verified']) && $this->request->data['User']['email_verified']) {
						...
						$this->Session->setFlash(__('Please check your mail and confirm your registration'),'message');
						$this->redirect('/register');
					}
				}
			}
		} else {
			$this->Session->setFlash(__('Sorry new registration is currently disabled, please try again later'),'message');
			$this->redirect('/login');
		}
	}
	/**
	...
	public function changePassword() {
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->RegisterValidate()) {
				...
				$this->Session->setFlash(__('Password changed successfully'),'message');
				$this->redirect('/dashboard');
			}
		}
	}
		...
		if (!empty($userId)) {
			...
			if ($this->request -> isPost()) {
				...
				if($this->User->RegisterValidate()) {
					...
					$this->User->save($user,false);
					$this->LoginToken->deleteAll(array('LoginToken.user_id'=>$userId), false);
					$this->Session->setFlash(__('Password for %s changed successfully', $name),'message');
					$this->redirect('/allUsers');
				}
			}
		} else {
	...
	public function addUser() {
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->RegisterValidate()) {
				...
				$this->request->data['User']['password'] = $this->UserAuth->makePassword($this->request->data['User']['password'], $salt);
				$this->User->save($this->request->data,false);
				$this->Session->setFlash(__('The user is successfully added'),'message');
	...
	public function editUser($userId=null) {
		if (!empty($userId)) {
			...
			if ($this->request -> isPut()) {
				...
				if ($this->User->RegisterValidate()) {
					$this->User->save($this->request->data,false);
					$this->Session->setFlash(__('The user is successfully updated'),'message');
					$this->redirect('/allUsers');
				}
			} else {
				$user = $this->User->read(null, $userId);
		...
		if (!empty($userId)) {
			if ($this->request -> isPost()) {
				if ($this->User->delete($userId, false)) {
					$this->LoginToken->deleteAll(array('LoginToken.user_id'=>$userId), false);
					$this->Session->setFlash(__('User is successfully deleted'),'message');
				}
			}
			$this->redirect('/allUsers');
		} else {
	...
	public function makeActiveInactive($userId = null, $active=0) {
		if (!empty($userId)) {
			...
			$this->User->save($user,false);
			if($active) {
				$this->Session->setFlash(__('User is successfully activated'),'message');
			} else {
				$this->Session->setFlash(__('User is successfully deactivated'),'message');
			}
		}
		$this->redirect('/allUsers');
	}
	...
	public function verifyEmail($userId = null) {
		if (!empty($userId)) {
			...
			$this->User->save($user,false);
			$this->Session->setFlash(__('User email is successfully verified'),'message');
		}
		$this->redirect('/allUsers');
	}
	/**
			...
			if (!empty($user)) {
				if (!$user['User']['email_verified']) {
					...
					if ($activateKey==$theKey) {
						...
						$this->Session->setFlash(__('Thank you, your account is activated now'),'message');
					}
				} else {
					$this->Session->setFlash(__('Thank you, your account is already activated'),'message');
				}
			} else {
				$this->Session->setFlash(__('Sorry something went wrong, please click on the link again'),'message');
			}
		} else {
			$this->Session->setFlash(__('Sorry something went wrong, please click on the link again'),'message');
		}
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->LoginValidate()) {
				...
				if (empty($user)) {
					$user = $this->User->findByEmail($email);
					if (empty($user)) {
						$this->Session->setFlash(__('Incorrect Email/Username'),'message');
						return;
					}
				}
				// check for inactive account
				if ($user['User']['id'] != 1 and $user['User']['email_verified']==0) {
					$this->Session->setFlash(__('Your registration has not been confirmed yet please verify your email before reset password'),'message');
					return;
				}
				$this->User->forgotPassword($user);
				$this->Session->setFlash(__('Please check your mail for reset your password'),'message');
				$this->redirect('/login');
			}
		}
	}
				...
				if ($this->User->RegisterValidate()) {
					...
					if (!empty($user)) {
						...
						if ($thekey==$activateKey) {
							...
							$this->User->save($user,false);
							$this->Session->setFlash(__('Your password has been reset successfully'),'message');
							$this->redirect('/login');
						} else {
							$this->Session->setFlash(__('Something went wrong, please send password reset link again'),'message');
						}
					} else {
						$this->Session->setFlash(__('Something went wrong, please click again on the link in email'),'message');
					}
				}
			} else {
				$this->Session->setFlash(__('Something went wrong, please click again on the link in email'),'message');
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->LoginValidate()) {
				...
				if($user['User']['email_verified']==0) {
					$this->User->sendVerificationMail($user);
					$this->Session->setFlash(__('Please check your mail to verify your email'),'message');
				} else {
					$this->Session->setFlash(__('Your email is already verified'),'message');
				}
				$this->redirect('/login');
			}
		}
